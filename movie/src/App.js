...
function App() {
  ...
  const getRating=(ratingValue)=> {
    setMovies(movies.filter(movie=>movie.rating===ratingValue));
  }
  const getSearch=(name)=>{
    setMovies(movies.filter(movie=>movie.name.includes(name)));
  }
